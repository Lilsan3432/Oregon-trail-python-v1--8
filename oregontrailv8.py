...
# Function to start the game and select initial parameters
def start_game():
...
by the end of the year, while managing your supplies,
health, and encountering various challenges along 
the way.
-------------------------------------------------
''')
...
''')
    ...
    # Prompt for purchasing supplies
    print("\nYou can now purchase additional supplies before starting your journey.")
    clothes_num = buy_supplies("clothes")
    ox_num = buy_supplies("oxen")
    ammo_num = buy_supplies("ammo")
    parts_num = buy_supplies("spare parts")

    print('\nStarting game...\n')
    return food_num, health_decrease_chance, clothes_num, ox_num, ammo_num, parts_num

# Function to handle buying supplies
def buy_supplies(item):
    while True:
        try:
            amount = int(input(f"Enter the amount of {item} you want to buy: "))
            if amount >= 0:
                return amount
            else:
...
# Main game loop
def main_game_loop(food_num, health_decrease_chance, clothes_num, ox_num, ammo_num, parts_num):
    # Game initialization
...
''')
    ...
    while player_move_distance < 2000 and food_num > 0 and health_num > 0 and month_num < 13:
        ...
        if choice == 'travel':
            ...
            ammo_needed = random.randint(5, 15)
            if ammo_num >= ammo_needed:
                food_gained = random.randint(50, 200)
                food_num += food_gained
                ammo_num -= ammo_needed
            ...
            print(f'Clothes - {clothes_num}, Oxen - {ox_num}, Ammo - {ammo_num}, Spare Parts - {parts_num}')
        elif choice == 'help':
            print('''
Instructions:
[travel]: moves you randomly between 30-60 miles and takes 3-7 days (random).
[rest]: increases health by 1 level (up to 10 maximum) and takes 2-5 days (random).
[hunt]: uses ammo to add between 50-200 lbs of food and takes 2-5 days (random).
[status]: displays current game status including supplies.
[quit]: ends the game.
            ''')
        elif choice == 'quit':
    ...
    if player_move_distance >= 2000:
        print_with_delay(f'Congratulations, you have successfully arrived in Oregon and started a new life!')
    elif food_num <= 0:
...
if __name__ == "__main__":
    food_num, health_decrease_chance, clothes_num, ox_num, ammo_num, parts_num = start_game()
    main_game_loop(food_num, health_decrease_chance, clothes_num, ox_num, ammo_num, parts_num)
